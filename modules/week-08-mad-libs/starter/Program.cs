...
public class Program
{
    ...
    private static StoryTemplate ChooseTemplate()
    {
        ...
        int templeteChoice = ReadIntInRange("Choose a template (1-2): ", 1, 2);
        if (templeteChoice == 1)
        {
            string[] inputPrompts =
            {
            ...
            return new StoryTemplate("Debugging at the Zoo", inputPrompts, "Today I visited the {0} zoo. I saw {1} {2} while writing {3}. I used a {4} {5} times until the bug disappeared. I felt {6} and yelled, \"{7}!\"");
        }
        else
        {
            string[] inputPrompts =
            ...
            return new StoryTemplate("The Standup Meeting", inputPrompts, "At the morning standup, the team waited as {0} confidently stepped forward, looking unusually {1}. Clutching a mysterious {2}, they proudly announced that they {3} through {4} stubborn {5}, only to be defeated at the last second by a sneaky {6}. Undeterred, they promised victory tomorrow—right after grabbing a well-earned {7}.");
        }
    }
